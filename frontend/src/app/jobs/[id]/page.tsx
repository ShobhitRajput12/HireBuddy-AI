    ...
    <div style={{ minHeight: '100vh', background: '#FDF8F4', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '960px', margin: '0 auto', padding: '60px 24px 80px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <span style={{ fontWeight: '700', fontSize: '16px', background: 'linear-gradient(135deg,#E8622A,#F5A623)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontFamily: 'Outfit, sans-serif' }}>Hirebuddy AI</span>
          ...
          <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
            <div style={{ marginBottom: '24px' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
                ...
                <span style={{ background: 'rgba(0,0,0,0.04)', color: '#78716C', border: '1px solid rgba(0,0,0,0.07)', fontSize: '11px', fontWeight: '600', padding: '4px 12px', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <Clock size={10} /> Posted {new Date(campaign.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </span>
              </div>
              ...
              {campaign.kanban_stage && campaign.kanban_stage !== 'Sourcing' ? (
                <div style={{ textAlign: 'center', padding: '40px 20px', background: 'rgba(0,0,0,0.02)', borderRadius: '14px', border: '1px solid rgba(0,0,0,0.05)' }}>
                  ...
                  <p style={{ fontSize: '13px', color: '#78716C', lineHeight: 1.5 }}>
                    We have moved to the evaluation stage for this role. Follow <strong>Hirebuddy AI</strong> for future opportunities.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleSubmit}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <div>
                      ...
                      >
                        <input ref={fileRef} type="file" hidden accept=".pdf,.doc,.docx" onChange={e => e.target.files && setResume(e.target.files[0])} />
                        {resume ? (
                          ...
                          <>
                            ...
                            <div style={{ fontSize: '11px', color: '#9CA3AF', marginTop: '3px' }}>PDF, DOC, DOCX · Max 10 MB</div>
                          </>
                        )}
                      </div>
